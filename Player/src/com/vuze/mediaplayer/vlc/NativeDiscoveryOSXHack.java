 ...
 * {@link LibVlcFactory#discovery(NativeDiscoveryOSXHack)}.
 * <p>
 ...
 */
public class NativeDiscoveryOSXHack {
     ...
     * Log.
     */
    private final Logger logger = LoggerFactory.getLogger(NativeDiscoveryOSXHack.class);
     ...
     */
    public NativeDiscoveryOSXHack(NativeDiscoveryStrategy... discoveryStrategies) {
            ...
            for(NativeDiscoveryStrategy discoveryStrategy : discoveryStrategies) {
                ...
                if(supported) {
                    ...
                    if(path != null) {
                        ...
                        String name = RuntimeUtil.getLibVlcLibraryName();
                        
                        // Hack due to https://github.com/caprica/vlcj/issues/643
                        if ( RuntimeUtil.isMac()){
	                        	try{
			                      NativeLibrary.addSearchPath(name+"core", path);
			                      Map<String,?> options = new HashMap<>();
			                      NativeLibrary.getInstance(name+"core", options );
	                        	}catch( Throwable e ){
	                        		e.printStackTrace();
	                        	}
                        }

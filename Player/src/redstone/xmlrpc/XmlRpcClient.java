...
public class XmlRpcClient extends XmlRpcParser implements XmlRpcInvocationHandler
{
    ...
    private void handleResponse() throws XmlRpcFault
    {
        try
        {
        	...
        	/*InputStream vzis = new InputStream( ) {
				InputStream internal = connection.getInputStream();
				
				public int read() throws IOException {
					int result = internal.read();
				...
				public int read(byte[] b, int off, int len) throws IOException {
					int result = internal.read(b,off,len);
					if(result != -1) {
						System.out.print(new String(b,off,result));
					}
					return result;
				}
        	...
        	InputStream vzis = connection.getInputStream();
        	
            parse( new BufferedInputStream( vzis ) );
        }
        catch ( Exception e )
        {
            throw new XmlRpcException(
